...
#Define variables
cnn_ds_path = './CNN/DS'
snn_ds_path = '/home/lizano/Documents/CSA-Loop/SNN/DS/'
...
memory = 2
window = 100
csv_df_name = 'Balanced-W'+str(window)+'-M'+str(memory)+'.csv'
csv_df_path = snn_ds_path+csv_df_name

#Create the models
cnn_model = img_class.createCNN(summary=False)
#snn_model = trayectory.createDNN(step,summary=True)
snn_model = trayectory.createRNN(memory,summary=True)
...
if snn_training_flag:
	...
	if os.path.isfile(weigth_file):
		os.remove(weigth_file)
	trayectory.trainModel(csv_df_path,snn_model,epochs=100,batch=4)
...
if snn_ds_create_flag:
	trayectory.createDS(snn_ds_path,window,memory)
